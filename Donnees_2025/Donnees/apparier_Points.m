...
vois1 = zeros(NbPix, nbptintI1);
vois2 = zeros(NbPix, nbptintI2);
h = floor(K/2);

for idx = 1:nbptintI1
    p1x = Ptint1(indptI1(idx), 1);
    p1y = Ptint1(indptI1(idx), 2);
    P = I1(p1y-h:p1y+h, p1x-h:p1x+h);   
    vois1(:, idx) = reshape(P, [], 1);
end

for idx = 1:nbptintI2
    p2x = Ptint2(indptI2(idx), 1);
    p2y = Ptint2(indptI2(idx), 2);
    P = I2(p2y-h:p2y+h, p2x-h:p2x+h);   
    vois2(:, idx) = reshape(P, [], 1);
end

% Utilisation de mean
meanVois1 = mean(vois1, 1);
meanVois2 = mean(vois2, 1);

% --- Centrage
vois1c = vois1 - meanVois1;    
vois2c = vois2 - meanVois2;
...
varVois1 = var(vois1, 0, 1);
varVois2 = var(vois2, 0, 1);
%%%%%%%%%%%%%%%%%
%% A COMPLETER %%
%%%%%%%%%%%%%%%%%
...
eps = 0.00001;
denom =  sqrt( varVois1(i1) .* varVois2(i2) ) * 1;   
cor = num ./ (denom * NbPix + eps);
%%%%%%%%%%%%%%%%%
%% A COMPLETER %%
%%%%%%%%%%%%%%%%%
